...
export const artistsSlice = createSlice({
    ...
    reducers: {
        // Todo: Falta agregar un filtro para realizar la barra de busqueda
        filterArtists: (state, action) => {
            console.log(action.payload)
            console.log(state.artists)
           const results = state.artists.filter(artist => artist.name.toLowerCase() === action.payload)
           console.log(results)
        }
    }
});

export const { filterArtists } = artistsSlice.actions;
